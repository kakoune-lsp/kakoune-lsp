...
[source,sh]
----
curl -O -L https://github.com/kak-lsp/kak-lsp/releases/download/v12.0.1/kak-lsp-v12.0.1-x86_64-apple-darwin.tar.gz
tar xzvf kak-lsp-v12.0.1-x86_64-apple-darwin.tar.gz
...
[source,sh]
----
wget https://github.com/kak-lsp/kak-lsp/releases/download/v12.0.1/kak-lsp-v12.0.1-x86_64-unknown-linux-musl.tar.gz
tar xzvf kak-lsp-v12.0.1-x86_64-unknown-linux-musl.tar.gz
...
* `lsp-diagnostics` command to list project-wide diagnostics (current buffer determines project and language to collect diagnostics for)
  * To pick a diagnostic from a menu, use the `<c-e>` mapping from the `lsp` user mode.
...
* `lsp-connect` to handle language server responses with a user-defined command.
...
| d | lsp-definition
| e | lsp-diagnostics
| <c-e> | lsp-connect lsp-goto-diagnostic lsp-diagnostics
| f | lsp-formatting
| h | lsp-hover
...
| r | lsp-references
| <c-r> | lsp-connect lsp-goto-reference lsp-references
| R | lsp-rename-prompt
