...
If you are using `lsp-stop-on-exit-enable` as proposed you might want to increase `editor.timeout`
(default value is 30 minutes) in `kak-lsp.toml` to not restart kak-lsp manually after being away
from long-running Kakoune session.
