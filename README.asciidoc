...
NOTE: Language server configuration must have the same key as correspondent filetype in Kakoune.
E.g. configuration for Rust language server must reside under `[language.rust]` section if you use default
`rust.kak` which sets `filetype` to `rust` for `*.rs` files. 
