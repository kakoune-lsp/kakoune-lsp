...
Some LSPs support inserting the inlay hints as text into your document. This can be useful when using `basedpyright` to write Python code, since it can make your type hints explicit. To do this, put the primary cursor next to the hint, and run `lsp-inlay-hint-apply-textedit`.
