...
determines project and language to collect diagnostics). Inline diagnostics use `DiagnosticError`
and `DiagnosticWarning` faces to highlight relevant parts of code depending on severity. To disable
inline diagnostics put `lsp-inline-diagnostics-disable` after `lsp-start` in your `kakrc`.
