    ...
    match report {
        DocumentDiagnosticReportResult::Report(report) => match report {
            DocumentDiagnosticReport::Full(full_report) => {
                ...
                // Update diagnostics the same way as publish diagnostics
                let mut diagnostics: Vec<_> = ctx
                    .diagnostics
                    .remove(buffile)
                    .unwrap_or_default()
                ...
                let new_diagnostics: Vec<_> = inner
                    .items
                    .iter()
                    .cloned()
                    .map(|d| (server_id, d))
        ...
        DocumentDiagnosticReportResult::Partial(_) => {
            // Partial results are not supported yet
            warn!(
                ctx.to_editor(),
            ...
            for item in report.items {
                match item {
                    WorkspaceDocumentDiagnosticReport::Full(full) => {
                        ...
                        diagnostics.extend(new_diagnostics);
                        ctx.diagnostics.insert(uri_str.clone(), diagnostics);
